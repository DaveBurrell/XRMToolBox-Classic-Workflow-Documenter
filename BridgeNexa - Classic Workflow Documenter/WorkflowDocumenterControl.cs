...
using BridgeNexa___Classic_Workflow_Documenter.Services;
using XrmToolBox.Extensibility;
using XrmToolBox.Extensibility.UserControls;

namespace BridgeNexa___Classic_Workflow_Documenter;
...
public sealed class WorkflowDocumenterControl : PluginControlBase
{
        ...
        WorkAsync(new WorkAsyncInfo
        {
            ...
            PostWorkCallBack = args =>
            {
                ...
                if (warningCount > 0)
                {
                    ...
                    ShowStatusNotification(message, true);
                }
                else
                {
                    ShowStatusNotification(message, false);
                }
    ...
    {
        row.BackColor = index % 2 == 0 ? Color.White : Color.FromArgb(248, 250, 254);
    ...
    private void ShowStatusNotification(string message, bool warning)
    {
        if (warning)
        {
            ShowWarningNotification(message, null, 10000);
            return;
        }

        ShowInfoNotification(message, null, 10000);
    }
